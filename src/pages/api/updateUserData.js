...
import {databaseQuery, printUserEntries} from '../../database';
...
export default (req, res) => {
    try {
        if (req.method === 'POST') {
          ...
          let api_key, userId, firstName, lastName, studentId;
          try{
            // ? Do we need to use an api key here ?
            api_key = req.body.api; // not used so far.
          ...
          let email, isEmailVerified;
          [email, isEmailVerified] = getEmailAndIsVerified(userId);
 ...
 * @param {string} mail The corresponding mail address (ger. 'E-Mail Adresse'). Cannot be empty.
 * @param {string} firstName The first name(s) of the user (ger. 'Vorname(n)'). Cannot be empty.
 * @param {string} lastName The sir name of the user (ger. 'Nachname'). Cannot be empty.
 * @param {string} studentId The student Id of the user (ger. 'Matrikelnummer').
...
function insertUser(userId, email, firstName = null, lastName = null, studentId = null, verified = false){
  ...
  let _userId, _email, _firstName, _lastName, _studentId, _verified;
  ...
  try {
    ...
    let params = [_userId, _email, _firstName, _lastName, _studentId, _verified];
